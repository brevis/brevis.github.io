  ...
  drawBeams(ctx, v) {
    ...
    if (b.off < 1 && v.x0 < b.x + 40 && v.x1 > b.x - 40) {
      for (let i = 0; i < b.pylons.length - 1; i++) {
        ...
        ctx.beginPath(); ctx.moveTo(b.x, y0 + 6); ctx.lineTo(b.x, y1 - 6); ctx.stroke();
        ctx.strokeStyle = `rgba(200,250,255,${0.85 * fl})`; ctx.lineWidth = 2.5;
        ctx.beginPath(); ctx.moveTo(b.x, y0 + 6);
        for (let k = 1; k < 6; k++) ctx.lineTo(b.x + rand(-3, 3), lerp(y0, y1, k / 6));
        ctx.lineTo(b.x, y1 - 6); ctx.stroke();
      }
    }
    for (const g of this.lgates) {
      if (!this.inView(v, g.x, (g.y0 + g.y1) / 2, 200)) continue;
      if (g.state === 'on') {
        ...
        ctx.beginPath(); ctx.moveTo(g.x, g.y0 + 8); ctx.lineTo(g.x, g.y1 - 8); ctx.stroke();
        ...
        ctx.beginPath(); ctx.moveTo(g.x, g.y0 + 8); ctx.lineTo(g.x, g.y1 - 8); ctx.stroke(); ctx.setLineDash([]);
      }
    }
    for (const l of this.rlasers) {
