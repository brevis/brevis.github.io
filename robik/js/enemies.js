...
const Enemies = {
  ...
  spawn(type, x, y, o = {}) {
    ...
    const e = {
      ...
      dead: false, dormant: false, dp: 9999,
    };
    G.enemies.push(e);
  ...
  spitter(e, dt, tgt, p) {
    ...
    if (e.charge > 0) {
      ...
      if (e.charge <= 0) {
        ...
        const aa = angTo(e.x, e.y, tgt.x + (tgt.tower ? 0 : p.vx * lead), tgt.y + (tgt.tower ? 0 : p.vy * lead));
        G.eBullets.push({ x: e.x + Math.cos(aa) * 16, y: e.y + Math.sin(aa) * 16, vx: Math.cos(aa) * 270, vy: Math.sin(aa) * 270, r: 8, dmg: e.dmg, life: 2.4, color: '#9be15d', towerDmg: true,
  ...
  turret(e, dt, tgt) {
    ...
    if (e.charge > 0) {
      ...
      e.burstT -= dt;
      if (e.burstT <= 0) {
        const aa = e.aim + rand(-0.06, 0.06);
        G.eBullets.push({ x: e.x + Math.cos(aa) * 28, y: e.y + Math.sin(aa) * 28, vx: Math.cos(aa) * 420, vy: Math.sin(aa) * 420, r: 6, dmg: e.dmg, life: 1.6, color: '#ff6a3c' });
        FX.flash(e.x + Math.cos(aa) * 28, e.y + Math.sin(aa) * 28, 16, '#ff9a5a', 0.08);
  ...
  draw(ctx, e) {
    ...
    if (e.type !== 'boss' && e.hp < e.maxHp && e.state !== 'emerge') {
      const w = e.type === 'brute' ? 80 : Math.max(26, e.r * 2), y = e.y - e.r - (e.type === 'brute' ? 22 : 12);
