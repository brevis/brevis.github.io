...
const World = {
  ...
  addLake(cx, cy, rx, ry, seed) {
    ...
    const shore = blobPoly(cx, cy, rx + 46, ry + 40, seed, 90, 0.22, 0.07);
  ...
  renderChunk(ci, cj) {
    ...
    // shores + water
    for (const l of this.lakes) if (ov(l.bb)) { g.fillStyle = pat('dirt'); g.fill(l.sPath); g.strokeStyle = 'rgba(60,30,8,0.35)'; g.lineWidth = 3; g.stroke(l.sPath); }
  ...
  drawWall(g, w, pat) {
    ...
    if (w.kind === 'cliff') {
      ...
      g.fillStyle = pat('rock'); g.fillRect(w.x, w.y, w.w, w.h);
