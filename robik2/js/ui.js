...
const SPEAKERS = {
  iskra: { name: 'ИСКРА', cls: 'art-iskra' },
  zhuzha: { name: 'ЖУЖА', cls: 'zhuzha art-zhuzha' },
  system: { name: 'СИСТЕМА', cls: 'system' },
  log: { name: 'ДАТА-ЛОГ', cls: 'log art-log' },
  boss: { name: 'КОЛОСС', cls: 'boss art-boss' },
};
...
const UI = {
  ...
  updateDialog(dt) {
    ...
    if (!this.cur && this.dq.length) {
      ...
      const who = this.cur.who;
      E.dName.style.color = who === 'zhuzha' ? 'var(--gold)' : who === 'system' || who === 'boss' ? '#ff6b5a' : who === 'log' ? 'var(--violet)' : 'var(--cyan)';
      E.dText.textContent = '';
  ...
  showLevelUp(p, choices, cb) {
    ...
    choices.forEach((u, i) => {
      ...
      d.innerHTML = `<div class="c-key">${i + 1}</div><div class="c-rar">${r.n}</div><div class="c-icon"><img src="assets/ic_${u.id}.webp" alt="" onerror="this.parentNode.textContent='${u.icon}'"></div>
  ...
  showPause(on) {
    ...
    if (on) {
      const p = G.player;
      const parts = UPGRADES.filter((u) => p.ups[u.id]).map((u) => `<span><img src="assets/ic_${u.id}.webp" alt="">${u.name} ×${p.ups[u.id]}</span>`);
