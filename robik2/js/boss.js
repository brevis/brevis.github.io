...
const Boss = {
    ...
    switch (e.bst) {
      ...
      case 'laser': {
        ...
        if (b.stage === 'tele') {
          ...
          G.addShake(0.08);
          const oy = this.oy(e);
          const len = this.rayLen(e.x, oy, b.a, 1100);
          b.len = len;
          const ex = e.x + Math.cos(b.a) * len, ey = oy + Math.sin(b.a) * len;
          if (Math.random() < 0.8) FX.sparks(ex, ey, b.a + Math.PI, 0.8, 2, '#ffd0c0', 150, 400, 0.2);
          if (p.alive && segDist2(p.x, p.y, e.x, oy, ex, ey) < (14 + p.hitR) ** 2) Player.hurt(p, 14, ex - Math.cos(b.a) * 300, ey - Math.sin(b.a) * 300, { kb: 200, iframes: 0.45 });
          for (const o of G.enemies) if (o !== e && !o.dead && segDist2(o.x, o.y, e.x, oy, ex, ey) < (14 + o.r) ** 2 && o.orbitCd < G.time) { o.orbitCd = G.time + 0.3; Enemies.damage(o, 20, { ang: b.a, kb: 200, quiet: true }); }
      ...
      case 'missiles': {
        ...
        if (e.shotT <= 0 && e.shots > 0) {
          ...
          G.eBullets.push({ x: e.x + Math.cos(a) * 50, y: this.oy(e) + Math.sin(a) * 50, vx: Math.cos(a) * 260, vy: Math.sin(a) * 260, r: 7, dmg: 14, life: 4.5, color: '#ff5a2a', kind: 'missile', homing: 2.3, shootable: true,
  ...
  pickAttack(e) {
    ...
    switch (atk) {
      ...
      case 'laser': {
        ...
        const a0 = angTo(e.x, this.oy(e), p.x, p.y) - dir * 1.0;
  ...
  oy(e) { return e.y + (e.hoy || 0); },

  shoot(e, a, sp, r, dmg, col, off = 46) {
    G.eBullets.push({ x: e.x + Math.cos(a) * off, y: this.oy(e) + Math.sin(a) * off, vx: Math.cos(a) * sp, vy: Math.sin(a) * sp, r, dmg, life: 4, color: col });
  },
  ...
  drawFx(ctx) {
    ...
    if (b) {
      ...
      const oy = this.oy(e);
      if (b.stage === 'tele') {
        const len = this.rayLen(e.x, oy, b.a0, 1100);
        ctx.strokeStyle = `rgba(255,60,60,${Math.floor(G.time * 16) % 2 ? 0.9 : 0.4})`; ctx.lineWidth = 2; ctx.setLineDash([16, 10]);
        ctx.beginPath(); ctx.moveTo(e.x, oy); ctx.lineTo(e.x + Math.cos(b.a0) * len, oy + Math.sin(b.a0) * len); ctx.stroke();
        ctx.setLineDash([]);
        ctx.strokeStyle = 'rgba(255,60,60,0.18)'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.moveTo(e.x, oy); ctx.arc(e.x, oy, 260, Math.min(b.a0, b.a0 + b.sweep), Math.max(b.a0, b.a0 + b.sweep)); ctx.closePath(); ctx.stroke();
      } else {
        const ex = e.x + Math.cos(b.a) * b.len, ey = oy + Math.sin(b.a) * b.len;
        ctx.globalCompositeOperation = 'lighter'; ctx.lineCap = 'round';
        ctx.strokeStyle = 'rgba(255,50,40,0.35)'; ctx.lineWidth = 46 + Math.sin(G.time * 60) * 6;
        ctx.beginPath(); ctx.moveTo(e.x, oy); ctx.lineTo(ex, ey); ctx.stroke();
  ...
  drawArt(ctx, e, wf, hitFlash) {
    const sleep = e.bst === 'sleep';
    const col = this.color(e);
    const s = e.scale || 1;
    const br = 1 + Math.sin(G.time * (e.phase === 3 ? 5 : 2)) * 0.012;
    const W = 270 * s, H = Art.h('boss', W), foot = e.y + 62;
    Art.shadow(ctx, e.x + 10, e.y + 48, 128 * s, 38 * s, 0.4);
    const tint = sleep ? ['#120a22', 0.55] : e.phase === 2 ? ['#ff2a10', 0.1] : e.phase === 3 ? ['#ff2ad0', 0.1 + Math.sin(G.time * 8) * 0.06] : null;
    Art.draw(ctx, 'boss', e.x, foot, W, { sx: br, sy: br, tint, flash: wf ? 1 : hitFlash ? 0.3 : 0 });
    if (!sleep) {
      const pulse = 1 + Math.sin(G.time * (e.phase === 3 ? 14 : 6)) * 0.15;
      const cy = foot - H * 0.55;
      ctx.save(); ctx.globalCompositeOperation = 'lighter';
      ctx.drawImage(FX.glow(col), e.x - 55 * pulse, cy - 55 * pulse, 110 * pulse, 110 * pulse);
      ctx.drawImage(FX.glow('#ffffff'), e.x - 12, cy - 12, 24, 24);
      ctx.restore();
    } else {
      ctx.save();
      ctx.font = "16px 'Russo One', sans-serif"; ctx.textAlign = 'center';
      ctx.fillStyle = `rgba(199,125,255,${0.4 + Math.sin(G.time * 2) * 0.3})`;
      ctx.fillText('z  z  z', e.x + 90, foot - H - 10 - Math.sin(G.time) * 6);
      ctx.restore();
    }
  },

  draw(ctx, e, wf, hitFlash) {
    if (Art.ready && Art.get('boss')) return this.drawArt(ctx, e, wf, hitFlash);
